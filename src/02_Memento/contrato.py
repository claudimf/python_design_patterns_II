...
class Historico(object):
    ...
    def lista_historico(self):
        lista = self.__estados_salvos
        estados = [
            self.obtem_estado(i).contrato.tipo for i, x in enumerate(lista)
            ]
        return estados
...
if __name__ == '__main__':
    ...
    print('Tipo do contrato: %s' % (contrato.tipo))

    contrato.avanca()
    historico.adiciona_estado(contrato.salva_estado())
    print('Tipo do contrato(avança 1): %s' % (contrato.tipo))

    contrato.avanca()
    historico.adiciona_estado(contrato.salva_estado())
    print('Tipo do contrato(avança 2): %s' % (contrato.tipo))

    contrato.avanca()
    historico.adiciona_estado(contrato.salva_estado())
    print('Tipo do contrato(avança 3): %s' % (contrato.tipo))

    print('Histórico: %s' % (historico.lista_historico()))

    contrato.restaura_estado(historico.obtem_estado(1))
    print('Tipo do resturado: %s' % (contrato.tipo))
